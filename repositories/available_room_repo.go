...
type AvailableRoomRepo interface {
	...
	SelectRoomsByBuildingID(context context.Context, tx *sql.Tx, date string, buildingId string) []domain.AvailableRoom
}
...
func (repo *AvailableRoomRepoImpl) SelectRoomsByBuildingID(context context.Context, tx *sql.Tx, date string, buildingId string) []domain.AvailableRoom {
	query := `
		SELECT
			f.id,
			f.floor_name,
	...
	rows, err := tx.QueryContext(context, query, date, buildingId)
	helper.PanicIfError(err)
	defer rows.Close()
